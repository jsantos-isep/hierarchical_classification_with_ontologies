...
def classify_chunk(texts, main_model, main_tokenizer, category_map, single_subcategory, sub_models):
    ...
    with torch.no_grad():
        ...
        main_pred_indices = torch.argmax(main_logits, dim=1).tolist()

    results = []

    for i, text in enumerate(texts):
        main_idx = main_pred_indices[i]
        main_category = category_map[main_idx]

        if main_category in single_subcategory:
            sub_label = single_subcategory[main_category]
            sub_idx = 0
        else:
            # Prepare single input for sub-model
        ...
        results.append({
            ...
            'sub_category_index': sub_idx,
            'sub_category_label': sub_label
        })

    return results

def process_dataset(args, ddf, total_rows, main_model, main_tokenizer, main_label_encoder, single_subcategory, category_map, sub_models):
    all_results = []
    ...
    for i, chunk in tqdm(enumerate(ddf), total=total_rows // size_per_chunk + 1, desc="Processing Chunks", mininterval=2):
        ...
        chunk_results = classify_chunk(texts, main_model, main_tokenizer, category_map, single_subcategory, sub_models)
...
if __name__ == "__main__":
    ...
    args = args_setup.get_arg_parser_with_evolution().parse_args()
    ...
    all_results = process_dataset(args, ddf, total_rows, classifier, classifier_tokenizer, main_label_encoder, single_subcategory, category_map, sub_models)
    ...
    precision_macro = precision_score(all_results['true_main_category'], all_results['main_category'], average='weighted', zero_division=0)
    recall_macro = recall_score(all_results['true_main_category'], all_results['main_category'], average='weighted', zero_division=0)
    ...
    recall_sub = recall_score(all_results['true_sub_category_label'], all_results['sub_category_label'], average='weighted',
                                zero_division=0)
    ...
    df_results = pd.DataFrame({'Main Accuracy': f"{main_accuracy * 100:.2f}%",
                               ...
                               "Main precision": f"{precision_macro * 100:.2f}%", "Main Recall": f"{recall_macro * 100:.2f}%",
    ...
    output_path = f"{os.getenv(f"DATASETS_FOLDER_{str(args.dataset).upper()}_RESULTS_FOLDER")}/{args.dataset}_{str(args.model_path).split('/')[-1]}_results_test_with_evolution.csv"
